function [x,f,exitflag,output] = minFunc_global(funObj,x0,options,tanMaxLength,x_lowlim,x_uplim,varargin)
...
%  
%   Partially modified by Yoshiaki Kariya, for global-LBFGS search.
        ...
        case LBFGS % L-BFGS
			...
			if Damped
				if i == 1
					...
					Hdiag = 1;
                elseif illegalCor
                    [fdammy,g] = funObj(x,varargin{:});
                    d = -g; % illegal cases use steepest descent direction
					old_dirs = zeros(length(g),0);
					old_stps = zeros(length(d),0);
					Hdiag = 1;
                else
                ...
                elseif  illegalCor
                    [fdammy,g] = funObj(x,varargin{:});
                    d = -g; % llegal cases use steepest descent direction
					S = zeros(p,corrections);
					Y = zeros(p,corrections);
					YS = zeros(corrections,1);
					lbfgs_start = 1;
...
%                    fprintf('Using negative curvature direction\n');
    ...
    d_ori=d;
    d= real(d);
    if sum(isnan(d(:))|isnan(d(:)))==length(d(:))
        d = (rand(size(d))-0.5)*10; %arbitrary
%        d=rand(size(d))*(log(x_uplim)-log(x_lowlim))+log(x_lowlim);  % arbitray
    end
    d(d==inf) = max(d(not(isinf(d(:))|isnan(d(:)))));
    d(d==-inf) = min(d(not(isinf(d(:))|isnan(d(:)))));
    d(isnan(d)) = (max(d(:))-min(d(:)))*rand(1)+min(d(:)); %
    if d~=d_ori
        illegalCor = true;
    else
        illegalCor =false;
  ...
  %globalization
  NonZeroSerch = true; 
  if i==1, globalization = false; end

  if (tanMaxLength ~= 0) && globalization 
    k_tv = 0;
    while NonZeroSerch
        if exist('tv_pre','var')==0
            tv_pre =1;
        end
        k_tv = k_tv+1;
        if k_tv >length(d)
        ...
        elseif d(k_tv) ~=0
           list=ones(size(d));
           list(k_tv)=0;
           listIndex = find(list ==1);
           d_tmp = d./norm(d);
%           tv(listIndex,1) =  rand(length(d)-1,1);
           tv(listIndex,1) =  (rand(length(d)-1,1)-0.5)*2;
           tv(k_tv) = -(d_tmp(listIndex,1))'*tv(listIndex,1)/d_tmp(k_tv,1);
           tv = tv./norm(tv);
           NonZeroSerch = false;
        end
        % following if section is actually meaningless due to line 1130
        if tv_pre ==1
        elseif dot(tv, tv_pre)<0
            tv = -tv;
        end
        tv_pre = tv;  % recording tv at previous iter
    ...
    size(d(1,1:(end-1)))
    size(tv_tmp)
%    tv_final = -(d(1:(end-1),1))'*tv_tmp/d(end,1);
    d_tmp = d./norm(d)
    tv_final = -(d_tmp(1,1:(end-1)))*tv_tmp/d_tmp(end,1);
    tv = [tv_tmp; tv_final]
    tv = tv./norm(tv)
%    d = d + tanMaxLength*(2*rand(1)-1) * tv;   % shift is defined as tangent against vector S.
%}
    d = d + (tanMaxLength*(2*rand(1)-1) * tv);
  end
    ...
    if LS_type == 0 % Use Armijo Bactracking
        ...
        %x = x.*not((x>log(x_uplim))) + (x>log(x_uplim)).*(log(x_uplim)-(0.5-rand(size(x)))*0.2*(log(x_uplim)-log(x_lowlim)));    % added on180625
        %x = x.*not((x<log(x_lowlim))) + (x<log(x_lowlim)).*(log(x_lowlim)+(0.5-rand(size(x)))*0.2*(log(x_uplim)-log(x_lowlim)));    % added on 180625
        %x = x.*not(isnan(x)) + isnan(x).*(rand(size(x))*(log(x_uplim)-log(x_lowlim))+log(x_lowlim));   % added on 180626
    elseif LS_type == 1 % Find Point satisfying Wolfe conditions
        ...
        % damping addition 200221
        k_damp=1;
        while k_damp < 11
            damp_check=(sum((x>log(x_uplim))+(x<log(x_lowlim))+(isnan(x))+(isinf(x)))~=0);
            if sum(damp_check)==0
                break
            end
            x = x-((1/2)^k_damp).*(t*d);
    ...
    % globalization
    if max(abs(t*d)) <= 1e-2 %progTol*1E8
        globalization = true;
    else
        globalization = false;
    end
